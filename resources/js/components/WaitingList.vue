<template>
  <div class="waiting-list-container" @click="closeAllDropdowns">
    ...
    <div class="user-grid">
      ...
      >
        ...
        <div class="action-buttons">
          <button class="action-btn view" @click.stop="openViewModal(user)">
          ...
          <button class="action-btn delete" @click.stop="openDeleteModal(user)">
    ...
    <div class="pagination-bar">
      <div class="page-info">Showing {{ currentPage }} of {{ totalPages }} pages ({{ filteredUsers.length }} users)</div>
    ...
    <!-- View Modal -->
    <div class="modal-overlay" v-if="viewModalVisible" @click.self="closeViewModal">
      <div class="modal-content-view">
        <div class="modal-header">
          ...
          <div class="header-text">
            ...
            <p class="modal-subtitle">view all the details of this entry</p>
          </div>
          ...
          <div class="detail-row">
            ...
            <div class="detail-column">
              <div class="detail-item">
                ...
                <span class="detail-value">{{ selectedUser.source }}</span>
              </div>
              <div class="detail-item">
                <span class="detail-label">Registered date & time</span>
                <span class="detail-value">{{ formatDate(selectedUser.joinDate) }} at {{ formatTime(selectedUser.joinDate) }}</span>
              </div>
            </div>
          </div>
          <div class="modal-actions">
            <button class="action-btn edit" @click.stop="editFromViewModal">
              <i class="ri-edit-line"></i>
              <span>Edit</span>
            </button>
            <button class="action-btn delete" @click.stop="deleteFromViewModal">
              <i class="ri-delete-bin-line"></i>
              <span>Delete</span>
            </button>
          </div>
    ...
    <!-- Edit Modal -->
    <div class="modal-overlay" v-if="editModalVisible" @click.self="closeEditModal">
      <div class="modal-content-edit">
        ...
        <div class="modal-body">
          ...
          <div class="modal-footer">
            <button class="modal-btn save-btn" @click="saveUserChanges">
    ...
    <div class="modal-overlay" v-if="deleteModalVisible" @click.self="closeDeleteModal">
      <div class="modal-content-delete">
        ...
        <div class="modal-body">
          <p class="warning-message">Are you sure you want to delete this entry? This action cannot be undone and will permanently remove all entry data.</p>
          <div class="modal-footer">
            <button class="modal-btn cancel-btn" @click="closeDeleteModal">Cancel</button>
...
export default {
  ...
  methods: {
    ...
    filterUsers() {
      this.filteredUsers = this.allUsers.filter(user => {
        ...
        const matchesDays = 
          ...
          this.daysBetween(user.joinDate, new Date()) <= this.selectedDays;
        
        const matchesTimeframe = 
          this.selectedTimeframe === null || 
          this.matchesTimeframe(user.joinDate, this.selectedTimeframe);
        
        const matchesStatus = 
          this.statusFilter === '' || 
          user.status === this.statusFilter;
        
        return matchesSearch && matchesSource && matchesDays && matchesTimeframe && matchesStatus;
      });
    ...
    async saveUserChanges() {
      try {
        this.loading = true;
        const response = await fetch(`/waiting-list/${this.selectedUser.id}/edit`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          if (index !== -1) {
            ...
            if (filteredIndex !== -1) {
              this.filteredUsers[filteredIndex] = this.allUsers[index];
            }
          }
    ...
    async confirmDelete() {
      try {
        this.loading = true;
        const response = await fetch(`/waiting-list/${this.selectedUser.id}/delete`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
