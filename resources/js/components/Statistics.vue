            ...
            <div class="stats-card">
                <div class="icon-container">
                    ...
                    >
                        ...
                        />
                        <path
                            d="M8 10H16V12H8V10ZM8 14H16V16H8V14ZM8 18H13V20H8V18Z"
                            fill="white"
                        />
                    </svg>
                </div>
...
export default {
    ...
    methods: {
        ...
        async fetchStatsData() {
            try {
                ...
                this.stats = {
                    ...
                    topSource: data.stats.top_source
                        ? data.stats.top_source.signup_source
                        : "None",
                };
            } catch (error) {
        ...
        async renderAllCharts() {
            ...
            try {
                if (
                    ...
                    !this.$refs.sourceChart ||
                    !this.$refs.sourceBarChart
                ) {
                ...
                this.cleanupCharts();
                this.showErrorNotification(
                    "Failed to render charts. Please try again."
                );
            }
        },
        ...
        async renderSourceDoughnutChart() {
            return new Promise((resolve) => {
                const sourceLabels = this.topSourcesData.map(
                    (item) => item.signup_source
                );
                const sourceData = this.topSourcesData.map(
                    (item) => item.total
                );
        ...
        downloadReport() {
            try {
                ...
                csvContent += `Days Filter,${
                    this.selectedDays ? this.selectedDays + "d" : "None"
                }\n`;
                csvContent += `Timeframe,${
                    this.selectedTimeframe
                        ? this.timeframes.find(
                              (t) => t.value === this.selectedTimeframe
                          ).label
                        : "None"
                }\n\n`;
...
.timeframe-dropdown div:first-child {
    font-weight: bold;
}
